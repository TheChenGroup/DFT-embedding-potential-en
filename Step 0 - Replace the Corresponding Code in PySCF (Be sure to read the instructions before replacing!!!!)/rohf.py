...
def get_occ(mf, mo_energy=None, mo_coeff=None, smear_sigma = None):
    ...
    if nelec[0] > nelec[1]:
        ...
        ncore, nocc = nelec
    nopen = nocc - ncore
    mo_occ = _fill_rohf_occ(mo_energy, mo_ea, mo_eb, ncore, nopen)[0]

    e_idx_a = numpy.argsort(mo_ea)
    e_sort_a = mo_ea[e_idx_a]
    e_homo_a = e_sort_a[nocc-1]
    e_lumo_a = e_sort_a[nocc]
#    print ('HOMO: ',e_homo, 'LUMO: ', e_lumo)
    ...
    if(smear_sigma > 1e-8):
        e_fermi_a, mo_occ_a = find_efermi(mo_ea, smear_sigma, nocc, nmo)
#        e_fermi_b, mo_occ_b = find_efermi(mo_eb, smear_sigma, nocc, nmo)
#        mo_occ_b = _fill_rohf_occ(mo_energy, mo_ea, mo_eb, ncore, nopen)[1]
        mo_occ_b = mo_occ - mo_occ_a
#        print ("Fermi energy_a: {e:18.10f}".format(e=e_fermi_a) )
    
    else:   
        mo_occ, mo_occ_b = _fill_rohf_occ(mo_energy, mo_ea, mo_eb, ncore, nopen)
        mo_occ_a = mo_occ - mo_occ_b
    ...
    return mo_occ, mo_occ_a, mo_occ_b
...
def _fill_rohf_occ(mo_energy, mo_energy_a, mo_energy_b, ncore, nopen):
    ...
    mo_occ[core_idx] = 2
    mo_occ[open_idx] = 1
    mo_occ_b = numpy.zeros_like(mo_energy)
    mo_occ_b[core_idx] = 1
    return mo_occ, mo_occ_b
...
class ROHF(hf.RHF):
    ...
    def init_guess_by_1e(self, mol=None):
        ...
        mo_occ = self.get_occ(mo_energy, mo_coeff)[0]
...
class HF1e(ROHF):
    def scf(self, *args):
        ...
        self.mo_occ = self.get_occ(self.mo_energy, self.mo_coeff)[0]
