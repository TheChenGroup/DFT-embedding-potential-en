...
Keyword argument "init_dm" is replaced by "dm0"''')
    ...
    for cycle in range(mf.max_cycle):
        ...
        mo_occ, mo_occ_a, mo_occ_b = mf.get_occ(mo_energy, mo_coeff, mf.smear_sigma)

        numpy.savetxt(os.path.join(mf.occ_path, 'hf_mo_occ.txt'), mo_occ)
        numpy.savetxt(os.path.join(mf.occ_path, 'hf_mo_occ_a.txt'), mo_occ_a)
        numpy.savetxt(os.path.join(mf.occ_path, 'hf_mo_occ_b.txt'), mo_occ_b)
    ...
    if scf_conv and conv_check:
        ...
        mo_energy, mo_coeff = mf.eig(fock, s1e)
        mo_occ = mf.get_occ(mo_energy, mo_coeff, mf.smear_sigma)[0]
...
def init_guess_by_huckel(mol):
    ...
    mo_occ = get_occ(SCF(mol), mo_energy, mo_coeff)[0]
...
#    print ('HOMO: ',e_homo, 'LUMO: ', e_lumo)
    print('LUMO-HOMO GAP = ', e_lumo - e_homo)
    e_fermi = e_homo
    ...
    mo_occ_a = mo_occ.copy()
    mo_occ_b = mo_occ.copy()
