...
def energy_elec(ks, dm=None, h1e=None, vhf=None):
    ...
    def energy_elec_before(ks, dm=None, h1e=None, vhf=None):
...
#                occ_i = mo_occ[i]/2.0 #closed shell
                if(mo_occ[i] > toll and mo_occ[i] < 1.0-toll):
                    S += mo_occ[i] * numpy.log(mo_occ[i]) + (1.0-mo_occ[i]) * numpy.log(1.0-mo_occ[i])
                else:
                    S += 0.0
    
        energy = S*smear_sigma
#        print ('entropy correction = ',energy)
    
        return energy
    
    tot_e, ecoul_exc = energy_elec_before(ks,dm,h1e,vhf) 

    hf_mo_occ_a = numpy.loadtxt(os.path.join(ks.occ_path, 'hf_mo_occ_a.txt'))
    hf_mo_occ_b = numpy.loadtxt(os.path.join(ks.occ_path, 'hf_mo_occ_b.txt'))
#    os.remove('/mnt/c/Users/19011/Desktop/hf_mo_occ.txt')
#    hf_mo_occ = numpy.array([1.97688754e+00, 2.31109116e-02, 1.54571866e-06, 4.13041202e-11])
    tot_e += entropy_corr(hf_mo_occ_a, ks.smear_sigma)
    tot_e += entropy_corr(hf_mo_occ_b, ks.smear_sigma)
...
def _dft_common_init_(mf, xc='LDA,VWN', smear_sigma = 0.0, occ_path = None):
    mf.xc = xc
    mf.nlc = ''
    mf.smear_sigma = smear_sigma   
    mf.occ_path = occ_path
...
def init_guess_by_vsap(mf, mol=None):
    ...
    # and guess density
    mo_occ = mf.get_occ(mo_energy, mo_coeff)[0]
